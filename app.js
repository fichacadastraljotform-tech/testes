...
function renderShell() {
  ...
  return `<div class="layout"><aside class="sidebar"><div class="brand"><img src="logo.png" alt="Logo Zelo" class="brand-logo"/><span>Zelo | Captação</span></div>${links.map(([id, label]) => `<button class="nav-btn ${session.route === id ? 'active' : ''}" data-route="${id}">${label}</button>`).join('')}<hr/><button class="nav-btn" id="logout">Sair</button></aside>
  <main class="main"><div class="topbar"><div class="topbar-left"><img src="logo.png" alt="Logo Zelo" class="header-logo"/><div><strong>${u.name}</strong><div class="muted">${u.role.toUpperCase()} • ${u.team}</div></div></div><div class="tag">Dia ${session.selectedDay}</div></div>${renderRoute()}</main></div>`;
}
